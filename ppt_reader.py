import streamlit as st
from pptx import Presentation
import pyttsx3
import tempfile
import os

# 음성 선택 옵션 (남성, 여성)
engine = pyttsx3.init()
voices = engine.getProperty('voices')

# 남성, 여성 음성 설정
male_voice = [voice for voice in voices if 'male' in voice.name.lower()]
female_voice = [voice for voice in voices if 'female' in voice.name.lower()]

# PPT 파일 업로드
st.title('PPT 대본 수정 및 음성 변환 웹앱')

uploaded_file = st.file_uploader("PPT 파일 업로드", type="pptx")
if uploaded_file is not None:
    presentation = Presentation(uploaded_file)

    # 각 슬라이드의 텍스트 추출 및 수정
    slide_scripts = {}
    for i, slide in enumerate(presentation.slides):
        text = ''
        for shape in slide.shapes:
            if hasattr(shape, 'text'):
                text += shape.text + "\n"
        # 대본 수정할 수 있도록 텍스트 박스 생성
        slide_scripts[i] = st.text_area(f"슬라이드 {i+1} 대본 수정", text)
    
    # 음성 선택 (남성, 여성)
    voice_choice = st.radio("음성 성별 선택", ("남성", "여성"))
    
    if voice_choice == "남성":
        selected_voice = male_voice[0] if male_voice else None
    else:
        selected_voice = female_voice[0] if female_voice else None
    
    if selected_voice:
        engine.setProperty('voice', selected_voice.id)
    else:
        st.warning("선택한 음성 옵션이 없을 수 있습니다. 기본 음성을 사용합니다.")
    
    # 음성 파일 생성
    if st.button("음성 파일 생성"):
        with tempfile.NamedTemporaryFile(delete=False) as temp_audio:
            for i, script in slide_scripts.items():
                temp_audio_path = temp_audio.name + f"_slide_{i+1}.mp3"
                
                # 선택한 음성으로 텍스트를 음성으로 변환
                engine.save_to_file(script, temp_audio_path)
                engine.runAndWait()
                
                # 생성된 음성 파일 재생
                st.audio(temp_audio_path)
                
            st.success("음성 파일이 생성되었습니다.")
